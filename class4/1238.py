...
def dijkstra(start):
    ...
    while q:
        # 하나씩 작은거 부터 뽑아서
        cnt_leng, cnt_node = heapq.heappop(q)

        # 만약에 dv가 너 크면 pass
        if cnt_leng < dv[cnt_node]:
            continue

        # 아니면 그 노드가 속한 간선과 이어진노드에서
        for adj_node, adj_leng in graph[cnt_node]:\
            # 거리 계산
            distance = adj_leng+ cnt_leng
            # 만약에 거리가 더 크면 
            if dv[adj_node] > distance:
...
ans=[]
# 여기서 가는 거리와 오는 거리가 다르므로 따로 계산
for i in range(1,N+1):
    ans.append(dijkstra(i)[X])
return_time = dijkstra(X)[1:N+1]
# 둘이 합쳐주고 max계산
for i in range(N):
    ans[i]=ans[i]+return_time[i]
