...
for i in range(N-1):
    mom_node,son_node,cost = map(int,input().split())
    graph[mom_node].append((cost, son_node))
    graph[son_node].append((cost, mom_node))
...
def dijkstra(start):
    ...
    # queue가 존재하는 동안
    while queue:
        cnt_cost, cnt_node = heapq.heappop(queue)
        # 만약 dv가 더 작으면 pass
        if dv[cnt_node] < cnt_cost: continue
        # 인접노드 불러와서 께산
        for adj_cost, adj_node in graph[cnt_node]:
            new_cost = cnt_cost + adj_cost
            # 계산 값이 더 작으면
            if dv[adj_node] > new_cost:
                dv[adj_node] = new_cost
              # 그걸로 갑시다
                heapq.heappush(queue,(new_cost,adj_node))
    return dv


# start_sum 임의의 수 하나 넣기
start_sum   = dijkstra(1)
# 거기서 가장 큰수의 인덱스 뽑아오기
start_point = start_sum.index(max(start_sum[1:]))
# 그걸 다시 함수에 넣기
ans         = max(dijkstra(start_point)[1:])
print(ans)
